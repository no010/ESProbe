...
#elif defined CONFIG_IDF_TARGET_ESP32C3
// NOTE: CSR GPIO instructions are not available in ESP-IDF 5.5.x toolchain.
// Using standard gpio_ll API instead.
#define SWCLK_SET() do { gpio_ll_set_level(&GPIO, PIN_SWCLK, 1); } while(0)
#define SWCLK_CLR() do { gpio_ll_set_level(&GPIO, PIN_SWCLK, 0); } while(0)
#define SWDIO_SET() do { gpio_ll_set_level(&GPIO, PIN_SWDIO_MOSI, 1); } while (0)
#define SWDIO_CLR() do { gpio_ll_set_level(&GPIO, PIN_SWDIO_MOSI, 0); } while (0)
#define SWDIO_GET_IN() \
  ({ \
    gpio_ll_get_level(&GPIO, PIN_SWDIO_MOSI) & 0x1; \
  })
#define SWDIO_OUT_ENABLE() do { gpio_ll_output_enable(&GPIO, PIN_SWDIO_MOSI); } while(0)
#define SWDIO_OUT_DISABLE() do { gpio_ll_output_disable(&GPIO, PIN_SWDIO_MOSI); } while(0)
#endif
